...
# 生成真实模型参数
true_slopes = np.random.randint(-10, 10, d)  # 真实权重
true_bias = 100  # 真实偏置（固定值）

# 生成目标值 y = X * true_slopes + true_bias + 高斯噪声
y = np.dot(x, true_slopes) + true_bias + np.random.normal(0, 100, dataSize)
...
for i in range(1, max_iters):
    ...
    w_history[i] = w.copy() / x_std  # 避免引用问题
    b_history[i] = b - np.sum(w * x_mean / x_std)
...
# ==================== 结果输出 ====================
print("\n训练结果:")
print(f"最优权重: {w/ x_std}")
print(f"最优偏置: {b - np.sum(w * x_mean / x_std):.2f}")
